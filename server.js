...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    const hasManual = manualTranscript && manualTranscript.trim();
    ...
    const topSegments = nonOverlapping
      ...
      .map((segment, index) => {
        return {
          ...
          ffmpegCommand: `ffmpeg -ss ${segment.start.toFixed(2)} -to ${segment.end.toFixed(2)} -i input.mp4 -vf "crop=ih*9/16:ih,scale=1080:1920" -c:a copy short-${index + 1}.mp4`
        };
      });
